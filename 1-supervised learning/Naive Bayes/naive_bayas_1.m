...
opts = struct('Optimizer', 'bayesopt', 'ShowPlots', true, 'CVPartition', cvpartition(YTrain, 'KFold', 5), 'AcquisitionFunctionName', 'expected-improvement-plus');

% Hiperparametre aralığını belirleyin
paramGrid = hyperparameters('fitcnb', XTrain, YTrain);
paramGrid(1).Range = [1, 100]; % 'Width' hiperparametresi için aralık

% Modeli eğitin ve optimize edin
nbModel = fitcnb(XTrain, YTrain, 'OptimizeHyperparameters', paramGrid, 'HyperparameterOptimizationOptions', opts);
...
legend('Setosa (Gerçek)', 'Versicolor (Gerçek)', 'Virginica (Gerçek)', 'Setosa (Tahmin)', 'Versicolor (Tahmin)', 'Virginica (Tahmin)', 'Location', 'Best');
