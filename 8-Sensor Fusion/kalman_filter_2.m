...
% Video oluşturma
video = VideoWriter('sensor_fusion.avi');
open(video);

% Grafik oluşturma
figure;
hold on;
h1 = plot(0, 0, 'r--', 'DisplayName', 'GPS X');
h2 = plot(0, 0, 'g--', 'DisplayName', 'Lidar X');
h3 = plot(0, 0, 'b-', 'DisplayName', 'Fused X');
xlabel('Time (s)');
ylabel('Position X (m)');
legend;
...
for k = 1:length(time)
    ...
    % Grafiği güncelleme
    set(h1, 'XData', time(1:k), 'YData', gps_position(1:k, 1));
    set(h2, 'XData', time(1:k), 'YData', lidar_position(1:k, 1));
    set(h3, 'XData', time(1:k), 'YData', estimated_positions(1:k, 1));
